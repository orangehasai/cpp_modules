...
void PhoneBook::searchContacts(void) const
{
	...
	for (int i = 0; i < _count; ++i)
	{
		...
		std::cout << std::setw(10) << formatColumn(_contacts[i].getFirstName()) << "|";
		std::cout << std::setw(10) << formatColumn(_contacts[i].getLastName()) << "|";
		std::cout << std::setw(10) << formatColumn(_contacts[i].getNickname()) << std::endl;
	}
	std::cout << "Index to display: ";
	if (!std::getline(std::cin, input))
		return;
	if (!parseIndex(input, &index) || index < 0 || index >= _count || !_contacts[index].isStored())
	{
